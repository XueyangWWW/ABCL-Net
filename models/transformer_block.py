...
class Upfold3D(nn.Module):
    ...
    def __init__(self, up_scale, in_channel=1, out_channel=64, patch=4, stride=1, padding=1):
        ...
        self.dim = in_channel
        self.in_dim = out_channel
    def forward(self, x, size=None, SC=None, reshape=True):
        # print(self.dim, self.in_dim)
        x = self.up_sample(x)
        if SC is not None:
...
class Upfold3D_seg(nn.Module):
    ...
    def __init__(self, up_scale, in_channel=1, out_channel=64, patch=4, stride=1, padding=1):
        ...
        self.dim = in_channel
        self.in_dim = out_channel
    def forward(self, x, size=None, SC=None, SC_G=None, reshape=True):
        x = self.up_sample(x)
        if SC is not None:
            if SC_G is not None:
                x = torch.cat((x, SC, SC_G), dim=1)
                x = x.flatten(start_dim=2).transpose(1, 2)
                print(x.size())
                # print(self.dim, self.in_dim)
                x = self.transformer(x)
